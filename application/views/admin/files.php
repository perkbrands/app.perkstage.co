            ...
            <div class="scroller-inner">
                ...
                <main id="contentContainer">
                    ...
                    <div class="row">
                        ...
                        <form action="<?php echo base_url(); ?>files/file_search" method="post" enctype="multipart/form-data" name="search_form" id="search_form">
                            <ul class="leftPaneFilters" >
                             ...
                             <li id="searchButtonLI">
                                ...
                                <li>
                                   ...
                                   <a href="<?php echo base_url(); ?>files/all_file"><li><button type="button" />View All</button></li></a>
                         </ul>
                         </form>
                            <div class="leftPaneFiltersDetails">
                            ...
                            <ul class="cardsBox">                                 
                               <?php  if(isset($query) && $query!='') { ?>                               	
                                 <a href="<?php echo base_url(); ?>files/file/new">
							     <li id="new_file" >
                                  <div class="plusSign">+</div>
                                    <div class="plusSignText">Upload File</div>
                                ...
                                <?php foreach($listing_data as $row){ ?>
 								<?php if(!empty($query)) {?> <li<?php if($row->file_id==$query[0]->file_id ) { ?> class="selected" <?php } ?>><?php } 								
								else {?> <li<?php if($row->file_id==$this->uri->segment(3)) { ?> class="selected" <?php }else{ ?> style="border:1px solid #535a60"<?php } ?>><?php } ?>            
                                    <?php if( $row->file_type=='') {?>
                                    	<div class="pill" style="background-image: url('<?php echo base_url(); ?>assets/img/admin/nofile.png');">&nbsp;</div>
									<?php } ?>	
									...
									<?php } else if( $row->file_type=='media'){?>
                                    <div class="pill" style="background-image: url('<?php echo base_url(); ?>assets/img/admin/media.png');">&nbsp;</div>
									<?php } ?>
                                    <p class="label"><?php echo substr($row->file_title,0,15).'...';?></p>
                                    
                                    <p class="options">
                                    <span id="setting_id"><?php echo "<a  href=".base_url()."files/file_setting/$row->file_id>Settings</a>
									"; ?></span>
                                  ...
                                  <!--  <p class="options"><span>-->
									...
									if(isset($row->file_profile_name) && $row->file_profile_name!='') {
									  echo "<a target = '_blank' href=".base_url().''.$file_path.'/'.$row->file_profile_name.">Open</a>"; ?></span></p>
                                    <?php } else {echo "<a target = '_blank'>Open</a>"; ?></span></p>
                                    <?php } ?>	
                        ...
                        <?php if(isset($query[0]->file_title)){ ?>
                            <h2 class="edit" id="edit"><?php echo $query[0]->file_title;?></h2>
                             <input type="hidden" id="file_id" name="file_id" value="<?php echo $query[0]->file_id; ?>" />
                            <?php } ?>
                            ...
                            <form action="<?php echo base_url(); ?>files/add_file" method="post" id="my_form" <?php  if(isset($query) && $query!='') { ?>  style="display:block;" <?php }else { ?> style="display:none;" <?php }  ?> enctype="multipart/form-data">
                            <?php if(!isset($query)){?>
                        <div style="float:left;margin:5px 0 20px 10px;width:90%">
                        <div class="labelInfo col span_4" style="color:#00A3E8" id="error_title">File Title</div>
                        <input style="width: 100%; height: 44px;" autocomplete="off" name="txt_file_title" id="title_id" onkeyup="titletext(this.value)">
                        </div>
                        <?php } ?>  
<!-- Status -->                            
                            ...
                            <h3  onclick="showSplitPlaneRow(this);">Status</h3><div class="splitPlaneQuickDetails" id="form_act_id"><?php if(isset($query[0])){ if(isset($query[0]->file_status) && $query[0]->file_status==1){echo 'Active';}else{echo 'Paused';}}else{echo 'Active';}?></div>
...
<!-- Profile -->                            
                                ...
                                <ul class="splitPaneRightDetails">
                                 <?php if(isset($query[0]->file_profile_name)){?>
                                    ...
                                    <?php if($query[0]->file_type=='document') {?>
									...
									<?php } else {?>
                                    <div style="float:left;padding-left:127px"><img src="<?php echo $query[0]->file_profile_path; ?>" 																							height="100" width="100" /></div>
                                   ...
                                   <?php } } ?> 
                                    <li class="row">
                                     <div id="loader" style="display:none;margin-left:125px">
   									<img src="<?php echo base_url()?>assets/img/admin/load.gif" />
  									</div>
                                         <div id="onsuccessmsg"></div>
                                    ...
                                    <li class="row" id="post_frame" style="display:none">
                                    ...
                                    <li class="row">
                                        <div class="labelInfo col span_4">Search Sumary</div>
                                    ...
                                    <div class="labelInfo col span_4">File Path</div>
                                     <div class="formInfo col span_12"><input type="text" id="file_path" readonly="readonly" name="" value="<?php if(isset($query[0]->file_profile_path)) echo $query[0]->file_profile_path; ?>"  /> </div>
                                    </li>
                                    <li class="row">
                                    <div class="labelInfo col span_4">File Size (kb’s)</div>
                                     <div class="formInfo col span_12"><input type="text" id="file_size" readonly="readonly" name="" value="<?php if(isset($query[0]->file_profile_size)) echo $query[0]->file_profile_size; ?>"  /></div>
                                    </li>
                                    <li class="row">
                                   <div class="labelInfo col span_4">Dimensions (px)</div>
                                  <div class="formInfo col span_12"><input type="text" id="file_dimention" readonly="readonly" name="" value="<?php if(isset($query[0]->file_profile_dimension)) echo $query[0]->file_profile_dimension; ?>" /></div>
                                    </li>
                                </ul>
                            </div>
<!-- Tags -->                            
...
<!-- Notes-->                            
                            <div class="splitPaneRightRow" >
                             ...
                             <input type="hidden" id="hidden_file_id" value="<?php if(isset($query[0]->file_id)) { echo $query[0]->file_id; } else{ echo $this->uri->segment(3); } ?>" name="hiddenid"/>   
...
$(document).ready(function() {
	...
	$('#remove_btn').attr("type", "button");
     <?php if($this->uri->segment(3)!='new'){ ?>
	  $('.edit').editable('<?php echo base_url()?>ajax/update_ajax_file', {
		 id : 'elementid',
         indicator : 'Saving...',
   ...
   <?php } ?>
	...
	if (r == true) {
		var del_id=$('#hidden_file_id').val();
		window.location.href='<?php echo base_url() ?>files/file_delete/'+del_id;
	} else {
...
function audience_filter(selectedValue)
 {
	 ...
	 {	
		$.ajax({
			url: '<?php echo base_url(); ?>files/file_filter',
			type: 'POST',
...
function onsuccess(response,status){
  ...
  $('#onsuccessmsg').html('<div id="msg" style="margin-left:127px">'+return_array[0]+'</div>');
	var kb_size=(return_array[2]/1000)
 	$('#file_path').val(return_array[1]);
 	$('#file_size').val(kb_size);
	var length=return_array.length;
	$('#file_dimention').val(return_array[4]);
	if(return_array[5]=='video')
	{
  		document.getElementById('post_frame').style.display='block';
	}
  }
...
$('#save_btn').click(function(){
	$('#my_form').attr('action', '<?php echo base_url();?>files/add_file');
});
