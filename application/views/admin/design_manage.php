...
<?php if(isset($version_data)){ $data_lenght= count($version_data); }// print_r($version_data[1]->design_version_id);?>
	<!-- Push Wrapper -->
    <div class="mp-pusher" id="mp-pusher">
        ...
        <nav id="mp-menu" class="mp-menu" >
            <div class="mp-level">
                <h2 class="icon icon-world">Design</h2>
                    ...
                    <div class="row">
                        <div id="splitPaneLeft" class="col span_8">
                            ...
                            <ul class="leftPaneFilters">
                                ...
                                <li id="searchButtonLI">
                                <form action="<?php echo base_url()?>designs/edit_source" method="post">
                                <input type="text" placeholder="Search"/>
                                <button id="searchButton" type="submit" name="searchButton" value=""></button>
                                </li>
		                	...
		                	<div class="codeBox">
		                        	<textarea id="source_area" cols="15" rows="25" name="text_source"><?php echo $html_data; ?></textarea>
									<input type="hidden" value="<?php if(isset($design_data_id) && $design_data_id != ''){ echo $design_data_id; } ?>" id="return_id" name="source_id" />
									<input type="hidden" id="type_id" name="source_type" value="html" />
									<input type="hidden"  name="texthidden"  value="<?php if(isset($query)) { echo $query[0]->design_id; } else { echo $this->uri->segment(3); } ?>" />
		                        </form>
							</div>
                        ...
                        <div class="col span_8" id="splitPaneRight">
                           ...
                           <?php if(isset($query[0]->design_title)){ ?>
                            ...
                            <?php } ?>
       						...
       						<form method="post" action="<?php echo base_url(); ?>designs/save_design" id="my_form" enctype="multipart/form-data">
<!-- Source -->                            
                                ...
                                <ul class="splitPaneRightDetails">
                                    <li class="row">
                                        ...
                                        <div class="formInfo col span_6">
											...
											{echo $version_data[$i]->content_type;?>
                                            
                                            <?php  if($version_data[$i]->content_type=='html') { ?>
                                             <option <?php if(isset($version_data[$i]->set_default) && $version_data[$i]->set_default==1){ echo 'selected=selected';} ?> value="<?php echo $version_data[$i]->design_data_id;?>||html"><?php echo $version_data[$i]->version; ?></option>
                                           <?php } } } ?>     
                                            </select>    
                                        </div>
                                    ...
                                    <li class="row">
                                        ...
                                        <div class="formInfo col span_6">
											...
											{?>
                                             ...
                                             <option <?php if(isset($version_data[$i]->set_default) && $version_data[$i]->set_default==1){ echo 'selected=selected';} ?> value="<?php echo $version_data[$i]->design_data_id; ?>||css"><?php echo $version_data[$i]->version; ?></option>
                                           <?php } } } ?>     
                                            </select>                                
                                        </div>
                                    ...
                                    <li class="row">
                                        ...
                                        <div class="formInfo col span_6">
                                        	...
                                        	<?php  if($version_data[$i]->content_type=='javascript') { ?>
                                             ...
                                             <option <?php if(isset($version_data[$i]->set_default) && $version_data[$i]->set_default==1){ echo 'selected=selected';} ?> value="<?php echo $version_data[$i]->design_data_id; ?>||javascript"><?php echo $version_data[$i]->version; ?></option>
                                           <?php } } } ?>     
                                            </select>                                
                                        </div>
...
<!-- Tags -->                            
                            <div class="splitPaneRightRow" >
                            <?php if(isset($query[0]->design_manage_tag)){
								$tag_design = rtrim($query[0]->design_manage_tag,',');
								$tags=explode(',',$tag_design);
								$tags_count=count($tags);
							}
								?>
                                <h3  onclick="showSplitPlaneRow(this);">Tags</h3><div class="splitPlaneQuickDetails" id="form_tag_id"><?php if(isset($query[0]->design_manage_tag) &&$query[0]->design_manage_tag!=''){ echo $tags_count;} else{echo '0';}?> Tags</div>
                                <ul class="splitPaneRightDetails">
                                    <li class="row">
                                        <div class="labelInfo col span_4">Tags</div>
                                        <div class="formInfo col span_12"><textarea name="txt_tag" class="doubleHeight"/><?php if(isset($query[0]->design_manage_tag)) echo $query[0]->design_manage_tag; ?></textarea></div>
                                    </li>
                                </ul>
                            </div>
<!-- Notes-->                            
                            <div class="splitPaneRightRow" >
                                ...
                                <ul class="splitPaneRightDetails">
                                   <li class="row">
                                    ...
                                    <div class="formInfo col span_12"><textarea name="txt_note" class="doubleHeight"/><?php if(isset($query[0]->design_manage_note)) echo $query[0]->design_manage_note; ?></textarea></div>
                                   </li>
                                </ul>
                            </div>
                             <input type="hidden" value="<?php if(isset($query)) { echo $query[0]->design_id; } else { echo $this->uri->segment(3); } ?>" name="hiddenid"/>   
                             <input type="hidden" name="design_hidden_id" value="<?php echo $this->uri->segment(3); ?>" />     
                            <div class="splitPaneRightRow rightButtonsRow row gutters">
								...
								{
						 		echo "<a  href=".base_url()."designs/design_manage_delete/$current_id>"; ?><button id="remove_btn" class="remove">Remove</button> </a> 
                                <?php } ?>
                                </div>
                                <div class="col span_4">&nbsp;</div>
                                <div class="col span_4">
									<button type="button" onclick="go_to_page()" id="cancel_btn" class="cancel">Cancel</button>
                                </div>
                                <div class="col span_4">                                    
...
$(document).ready(function() {
	...
	$('#remove_btn').attr("type", "button");
	  ...
	  $('.edit').editable('<?php echo base_url()?>ajax/update_ajax_design_manage', {
		 id : 'elementid',
         indicator : 'Saving...',
...
function onsuccess(response,status){
 ...
 $('#edit_source').click(function(){
	...
	$.ajax({
			url: '<?php echo base_url(); ?>designs/select_source',
			type: 'POST',
...
$('#save_btn').click(function(){
	$('#my_form').attr('action', '<?php echo base_url();?>designs/save_design');
});
...
function go_to_page(){
window.location.href="<?php echo base_url().'designs';?>";	
}
</script>
